...
const Helsinki = ({Helsinki}) => {
    ...
    useEffect(() => {
        ...
        if (date ==="3"){
            fetch(`https://api.open-meteo.com/v1/forecast?latitude=60.17&longitude=24.94&hourly=temperature_2m&daily=weathercode,temperature_2m_max,temperature_2m_min&current_weather=true&timezone=auto&start_date=${startDate}&end_date=${endDate}`)
            .then(response => response.json())
            .then(json => setData(json))
        }
    },[date])
