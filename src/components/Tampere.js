...
const Tampere = ({Tampere}) => {
    ...
    useEffect(() => {
        if (date === "1"){
            fetch("https://api.open-meteo.com/v1/forecast?latitude=61.50&longitude=23.79&current_weather=true")
            .then(response => response.json())
            .then(json => setData(json))
        }
        if (date ==="3"){
            fetch(`https://api.open-meteo.com/v1/forecast?latitude=61.50&longitude=23.79&hourly=temperature_2m&daily=weathercode,temperature_2m_max,temperature_2m_min&current_weather=true&timezone=auto&start_date=${startDate}&end_date=${endDate}`)
            .then(response => response.json())
            .then(json => setData(json))
        }
    },[date])
